...
- Always runs `ops/finalize-run.ps1` once the spec finishes -- pass, fail, or an unexpected runner error -- to kill stray processes (`devenv`, `MSBuild`, `ServiceHub*`, `vshost`, `notepad`) and delete leftover project/artifact folders in `$HOME`. Scoping is manifest-backed, not just "created/started during the current run": a persisted state file (`%LOCALAPPDATA%\ui-automation\cleanup-state.json`) remembers the end time of the last cleanup attempt and any specific process/path a previous attempt already failed to remove, so a leftover from an earlier run whose own cleanup never got to execute (e.g. a crash) is still caught, and a resource stuck behind a transient failure (e.g. access denied) is retried on every subsequent invocation until it's actually gone -- while a pre-existing, unrelated IDE/process/project is never touched. It intentionally does **not** terminate `conhost`/`cmd` in this mode (their process ancestry can't be safely scoped) -- those are left to each spec's own `Clean up` steps and the on-failure window-close above. Pass `--no-cleanup` to skip this entirely (e.g. when debugging a failure's leftover state).

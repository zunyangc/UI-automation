...
- Always runs `ops/finalize-run.ps1` once the spec finishes -- pass, fail, or an unexpected runner error -- to kill stray processes (`devenv`, `MSBuild`, `ServiceHub*`, `vshost`, `notepad`) and delete leftover project/artifact folders in `$HOME`, scoped to resources created/started at or after this run's own start (`-Since`), so it doesn't touch a pre-existing, unrelated IDE/process/project -- including one a user creates independently during idle time between specs. Before running the spec, `run_test.py` also checks a small marker file (`%LOCALAPPDATA%\ui-automation\active-run-marker.json`) left by a *previous* run: if that run crashed before reaching its own cleanup, the marker is still present, and this run extends its own `-Since` back to that earlier run's start so its genuine leftovers are still caught, without ever sweeping in unrelated idle-time activity. Separately, a persisted retry manifest (`%LOCALAPPDATA%\ui-automation\cleanup-state.json`) tracks any specific process/path a previous cleanup attempt already targeted but failed to remove (e.g. transient access denied), and retries it unconditionally on every subsequent invocation until it's actually gone. It intentionally does **not** terminate `conhost`/`cmd` in this mode (their process ancestry can't be safely scoped) -- those are left to each spec's own `Clean up` steps and the on-failure window-close above. Pass `--no-cleanup` to skip this entirely (e.g. when debugging a failure's leftover state) -- note this also leaves the active-run marker in place, so a later real cleanup run still recognizes this run's resources as unfinished business.
